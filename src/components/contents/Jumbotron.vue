<script>
export default {
    name: 'Jumbotron'
}
</script>
<template>
    <div class="jumbotron container-none">
        <div class="title uppercase mb-5">
            <h1 class="text-5xl font-bold">Serena Dong</h1>
            <p class="text-xl font-bold">Junior Full Stack Web Developer</p>
        </div>
        <div class="content">
            <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Sunt consequatur, optio voluptate iusto, nemo
                natus deleniti excepturi beatae non obcaecati totam eius facilis ipsa eum maxime temporibus nihil
                adipisci quaerat.</p>
        </div>
    </div>
</template>
<style scoped lang="scss">
.jumbotron {
    padding: 2rem 0;

}
</style>